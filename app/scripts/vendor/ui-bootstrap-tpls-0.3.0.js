    ...
    .directive('accordionGroup', ['$parse', '$transition', '$timeout', function($parse, $transition, $timeout) {
        return {
            ...
            link: function(scope, element, attrs, accordionCtrl) {
                var getIsOpen, setIsOpen, getIsActive, setIsActive;
                ...
                if ( attrs.isActive ) {
                    getIsActive = $parse(attrs.isActive);
                    setIsActive = getIsActive.assign;

                    scope.$watch(
                        function watchIsActive() { return getIsActive(scope.$parent); },
                        function updateActive(value) { scope.isActive = value; }
                    );

                    scope.isActive = getIsActive ? getIsActive(scope.$parent) : false;
                }
    ...
    ['$document', '$location', '$window', function ($document, $location, $window) {
        ...
        return {
            ...
            link: function(scope, element, attrs) {
                ...
                element.parent().bind('click', function(event) { closeMenu(event); });
                    ...
                    if (!elementWasOpen){
                        ...
                        closeMenu = function (event) {
                            if ((event && event.toElement
                                ...
                                && event.toElement && event.toElement.tagName !== 'SELECT'
                                && event.toElement && event.toElement.tagName !== 'TEXTAREA'
...
angular.module("template/accordion/accordion-group.html", []).run(["$templateCache", function($templateCache) {
    $templateCache.put("template/accordion/accordion-group.html",
        "<div class=\"accordion-group\">\n" +
            "  <div class=\"accordion-heading\" ><a class=\"accordion-toggle {{!isActive && 'muted' || 'active'}} {{!isOpen && 'closed' || 'open'}}\" ng-click=\"isOpen = isActive && !isOpen\" accordion-transclude=\"heading\">{{heading}}</a></div>\n" +
